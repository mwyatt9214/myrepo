...
class Model():
	def __init__(self,year,leagueID,cookie1,cookie2):
		...
		self.tradeCosts = 0
		self.numKept = 0
		self.scoring  = {'pYards':0.04,'Int':-2,'pTD':4,'p2PT':2,'rshYards':0.1,'rshTD':6,'rsh2PT':2,'recYards':0.1,'recTD':6,'rec':0.5,'rec2PT':2,'KRTD':6,
	...
	def updateProj(self,fileName):
		...
		tempPQB = self.projQB.ix[:,1:-2]
		tempPRB = self.projRB.ix[:,1:-2]
		tempPWR = self.projWR.ix[:,1:-2]
		tempPTE = self.projTE.ix[:,1:-2]
